...
namespace HBBio.Collection
{
    ...
    public class CollectionValve
    {
        ...
        /// <returns></returns>
        public bool JudgeCondition(ref int index, double[] listTVCV, List<double> listVal, List<double> listMinVal, List<double> listMaxVal, List<double> listSlope, List<double> listLastSlope)
        {
            if (m_signal)
            {
                //判断条件1
                            ...
                            case EnumPositionType.Fixed:
                                ...
                                switch (m_list[m_condIndex].MPositionStart)
                                {
                                    ...
                                    case EnumPositionStart.Out:
                                        switch (EnumOutInfo.NameList[1])
                                        {
                                            case "Out1":
                                                QueAddCond(m_list[m_condIndex].MStartIndex, listTVCV[1], m_list[m_condIndex]);
                                                break;
                                            default:
                                                QueAddCond(m_list[m_condIndex].MStartIndex - 1, listTVCV[1], m_list[m_condIndex]);
                                                break;
                                        }
                                        break;
                                }
        ...
        /// <param name="listVal"></param>
        /// <param name="listSlope"></param>
        private void JudgeLoop(int index, double[] listTVCV, List<double> listVal, List<double> listMinVal, List<double> listMaxVal, List<double> listSlope, List<double> listLastSlope)
        {
            ...
            if (first)
            {
                switch (m_list[m_condIndex].MPositionStart)
                {
                    ...
                    case EnumPositionStart.Out:
                        ...
                        {
                            case "Out1":
                                m_currIndex = m_list[m_condIndex].MStartIndex;
                                break;
                            default:
