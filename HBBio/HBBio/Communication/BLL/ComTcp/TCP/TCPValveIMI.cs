...
{
    class TCPValveIMI : TCPValve
    {
        /// <summary>
        /// 构造函数
        /// </summary>
        public TCPValveIMI(ComConf info) : base(info)
        {
        ...
        public override bool ReadVersion(ref string version)
        {
            ...
            if (0x15 == m_ReadByte[0])
            {
                version = Encoding.Default.GetString(m_ReadByte, 0, 1);
                return true;
            }
            else if (0x30 == m_ReadByte[0] && 0x32 == m_ReadByte[1])
            {
                version = Encoding.Default.GetString(m_ReadByte, 0, 2);
                return true;
            }
